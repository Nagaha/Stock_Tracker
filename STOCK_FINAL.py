...
def delete():
    ...
    if not name:
        ...
        del_query = "DELETE FROM stock_info WHERE stock_name =%s"
        mycursor.execute(del_query, (name,))
        mydb.commit()
...
generate_graph_btn = ctk.CTkButton(tab1, text='GENERATE GRAPH', font=('sans', 15, 'bold'), fg_color='green', width=10,
                                   command=generate_graph).place(x=150, y=400)
generate_report_btn = ctk.CTkButton(tab1, text='GENERATE REPORT', font=('sans', 15, 'bold'), fg_color='green', width=10,
...
clear_btn = ctk.CTkButton(tab1, text='CLEAR', font=('sans', 15, 'bold'), fg_color='green', width=20,
